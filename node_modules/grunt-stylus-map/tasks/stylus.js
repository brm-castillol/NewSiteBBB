...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('stylus', 'Compile Stylus files into CSS', function() {
    ...
    async.forEachSeries(this.files, function(f, n) {
      ...
      async.concatSeries(srcFiles, function(file, next) {
        compileStylus(file, options, function(res, err) {
          if (!err) {
            compiled.push(res.css);

            if(res.sourcemap){
              map.push(res.sourcemap);
            }
        ...
        if (compiled.length < 1) {
          ...
          if(map.length == 1){
            grunt.file.write(destFile + ".map",map[0]);
          }
          else{
            console.log("No map file created");
          }
  ...
  var compileStylus = function(srcFile, options, callback) {
    ...
    s.render(function(err, css) {
      ...
      if(s.sourcemap){
        res.sourcemap = JSON.stringify(s.sourcemap);
      }

      callback(res, err ? err : false);
